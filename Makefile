...
COMMON_FLAGS = -D__EMSCRIPTEN__ -sSUPPORT_LONGJMP=emscripten -pipe -pthread -fopenmp \
...
LINK_FLAGS = -DQUAD $(LDFLAGS) -sDEFAULT_TO_CXX=1 -sALLOW_TABLE_GROWTH=1 -sEMULATE_FUNCTION_POINTER_CASTS=0 -sSUPPORT_BIG_ENDIAN=0 \
	 -sWASM_BIGINT=1 -sOFFSCREENCANVAS_SUPPORT=1 -pthread -sDEFAULT_PTHREAD_STACK_SIZE=8mb -openmp -sMALLOC='mimalloc' \
	 ...
	 -sALLOW_MEMORY_GROWTH=0 -sMAXIMUM_MEMORY=4gb -sINITIAL_MEMORY=1024mb --output_eol linux -mllvm -wasm-enable-eh \
...
b3_cropcircle:
	 ...
	 -sFORCE_FILESYSTEM=1 --bind -lembind -mtune=wasm32 -sASYNCIFY=0 -sASYNCIFY_STACK_SIZE=81920 \
