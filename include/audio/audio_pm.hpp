...
void audio_module_set_js_pcm_handler(emscripten::val handler_function) {
    if (handler_function.isFunction()) {
        js_pcm_data_handler = handler_function;
        printf("AudioModule: JS PCM data handler has been set.\n");
    } else {
        fprintf(stderr, "AudioModule: Provided handler is not a function.\n");
    }
}
